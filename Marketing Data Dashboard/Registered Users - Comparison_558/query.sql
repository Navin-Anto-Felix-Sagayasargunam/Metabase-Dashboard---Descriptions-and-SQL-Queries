...
-- Dashboard ID: 50
-- Chart: Registered Users - Comparison
-- Card ID: 558
-- Query Type: native
-- Database ID: 2
...
WITH monthly_counts AS (
    SELECT
        ...
        COUNT(*) AS user_count
    FROM customers_v4
    WHERE creation_date::date >= COALESCE({{start_date}}, DATEADD(month, -12, DATE_TRUNC('month', CURRENT_DATE)))
      AND creation_date::date < DATE_TRUNC('month', CURRENT_DATE)  -- only completed months
