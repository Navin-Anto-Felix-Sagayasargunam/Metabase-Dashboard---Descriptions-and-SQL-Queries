...
-- Dashboard ID: 50
-- Chart: Active Users - Previous 90 days
-- Card ID: 230
-- Query Type: native
-- Database ID: 2
...
where
((
    last_activity_date::Date >= TO_CHAR(CURRENT_DATE - INTERVAL '90 day', 'YYYY-MM-DD') --CURRENT_DATE - INTERVAL '30 day'
    ...
    (
        (
  TO_TIMESTAMP(created_at, 'YYYY-MM-DD"T"HH24:MI:SS.MS')::date >= TO_CHAR(CURRENT_DATE - INTERVAL '90 day', 'YYYY-MM-DD')
  AND TO_TIMESTAMP(created_at, 'YYYY-MM-DD"T"HH24:MI:SS.MS')::date <= CURRENT_DATE
    )
   ));
--WHERE date_trunc('day', se.last_activity_date) 
 --     BETWEEN date_trunc('day', CURRENT_DATE - INTERVAL '90 day')
